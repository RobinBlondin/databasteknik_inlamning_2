...
public class ScrollableGridLayout extends JPanel {
    ...
    public ScrollableGridLayout(MainFrame mainFrame, Reporter reporter, Repository repo) {
    ...
    public void refresh() {
        ...
        for (String str : filteredList) {
            ...
            gridPanel.add(new ListPanel(arr[0], arr[1], arr[2], String.valueOf(arr[3]), String.valueOf(arr[4])));
        }
    ...
    public void removeFromUI(String platform, String userName) {
        for (Component component : gridPanel.getComponents()) {
            if (component instanceof ListPanel listPanel) {
                if (listPanel.getBrandText().equalsIgnoreCase(platform) && listPanel.getModelText().equalsIgnoreCase(userName)) {
